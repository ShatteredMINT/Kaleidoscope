...
static void HandleDefinition() {
  if (auto FnAST = Parser::ParseDefinition()) {
    if (auto *FnIR = FnAST->codegen()) {
      ...
      JIT::ExitOnErr(JIT::TheJIT->addModule(llvm::orc::ThreadSafeModule(
          std::move(IR::Module), std::move(IR::Context))));
...
static void HandleTopLevelExpression() {
  if (auto FnAST = Parser::ParseTopLevelExpr()) {
    if (auto *FnIR = FnAST->codegen()) {
      ...
      // add finished module containing expression and create new module for
      // future code
      auto TSM = llvm::orc::ThreadSafeModule(std::move(IR::Module),
                                             std::move(IR::Context));
...
void printHelp() {
  ...
  printf("-i\t--include-file\t\tspecify a file with kaleidoscope code to "
         "include / execute before starting the console\n");
