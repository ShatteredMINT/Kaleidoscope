...
std::unique_ptr<AST::ExprAST> Parser::ParseIdentifierExpr() {
  ...
  while (Lexer::CurTok != ')') {
    ...
    if (Lexer::CurTok != ',' && Lexer::CurTok != ')')
      return Log::LogError<std::unique_ptr<AST::ExprAST>>(
  ...
  //
  //         if (Lexer::CurTok != ',')
  //             return Log::LogError("test: Expected ')' or ',' in argument
  //             list");
  //         Lexer::getNextToken();
  //     }
  // }
...
Parser::ParseBinOpRHS(int ExprPrec, std::unique_ptr<AST::ExprAST> LHS) {
  while (true) {
...
std::unique_ptr<AST::PrototypeAST> Parser::ParsePrototype() {
  ...
  switch (Lexer::CurTok) {
  default:
    return Log::LogError<std::unique_ptr<AST::PrototypeAST>>(
